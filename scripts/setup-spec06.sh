...
show_help() {
    ...
    echo "Options:"
    echo "  --spec-dir PATH         设置SPEC路径 (默认: /SPECcpu2006)"
...
if ${SPEC_REBUILD}; then
    SPEC_COMMEND="runspec -c ${SPEC_CFG} -a ${SPEC_ACTION} --rebuild -T ${SPEC_TUNE} -n ${SPEC_ITERATIONS} -i ${SPEC_INPUT} ${SPEC_BENCHES}"
else
    SPEC_COMMEND="runspec -c ${SPEC_CFG} -a ${SPEC_ACTION} -T ${SPEC_TUNE} -n ${SPEC_ITERATIONS} -i ${SPEC_INPUT} ${SPEC_BENCHES}"
fi
