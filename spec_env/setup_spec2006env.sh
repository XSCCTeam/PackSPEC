...
# 默认配置
DEFAULT_SPEC2006_URL="http://172.38.10.144/api/v4/projects/65/packages/generic/cpu2006/1.2/cpu2006-1.2.iso"
...
# 配置变量
SPEC2006_URL="${SPEC2006_URL:-$DEFAULT_SPEC2006_URL}"
...
# 下载ISO文件
download_iso() {
    log "$INFO_LABEL" "准备下载SPEC CPU 2006 ISO文件..."
    
    if [ -f "$SPEC2006_ISO" ]; then
        log "$ATTENTION_LABEL" "ISO文件 '$SPEC2006_ISO' 已存在，跳过下载"
        return 0
    fi
    
    log "$INFO_LABEL" "从 $SPEC2006_URL 下载 $SPEC2006_ISO..."
    
    if ! wget -q "$SPEC2006_URL" -O "$SPEC2006_ISO" --show-progress; then
...
# 清理临时文件
cleanup() {
    log "$INFO_LABEL" "是否清理ISO文件？(y/N) [5秒后自动选择N]"
    ...
    if [[ $read_status -eq 0 ]] && [[ "$cleanup_iso" =~ ^[Yy]$ ]]; then
        if rm -f "$SPEC2006_ISO"; then
            log "$SUCCESS_LABEL" "ISO文件已清理"
        else
            log "$ERROR_LABEL" "清理ISO文件失败"
        fi
    else
        # 无论是超时还是用户输入非y，都显示保留信息
        if [[ $read_status -eq 124 ]]; then
            log "$INFO_LABEL" "超时，自动选择保留ISO文件"
        else
...
show_help() {
    ...
    echo "选项:"
    echo "  -h, --help            显示此帮助信息"
    echo "  -u, --url URL         指定SPEC2006 ISO文件的下载URL"
    echo "  -i, --iso FILE        指定ISO文件名（默认: $DEFAULT_SPEC2006_ISO）"
    echo "  -d, --dir DIR         指定安装目录（默认: $DEFAULT_SPEC2006_INSTALL_DIR）"
    echo ""
    echo "环境变量:"
    echo "  SPEC2006_URL          与 -u 选项相同"
